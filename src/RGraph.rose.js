...
RGraph.Rose = function(conf) {
  ...
  this.draw = function() {
    ...
    if (properties.segmentHighlight) {
      ...
      if (!RGraph.allowSegmentHighlight) {
        alert(
          "[WARNING] The segment highlight function does not exist - have you included the dynamic library?"
        );
      }
      ...
      RGraph.allowSegmentHighlight({
        object: this,
        count:
          typeof properties.segmentHighlightCount === "number"
            ? properties.segmentHighlightCount
            : this.data.length,
  ...
  this.drawBackground = function() {
    ...
    if (properties.backgroundGridCirclesCount) {
      if (typeof properties.backgroundGridCirclesCount == "number") {
        properties.backgroundGridCirclesSize =
          this.radius / properties.backgroundGridCirclesCount;
      }
      ...
      for (
        var i = properties.backgroundGridCirclesSize;
        i <= this.radius;
        i += properties.backgroundGridCirclesSize
      ) {
        // Hmmm... This is questionable
        this.context.moveTo(this.centerx + i, this.centery);

        // Radius must be greater than 0 for Opera to work
        this.context.arc(this.centerx, this.centery, i, 0, RGraph.TWOPI, false);
      }
      this.context.stroke();
    ...
    if (properties.axes) {
      ...
      if (properties.axesTickmarks) {
        // Draw the X ends
        this.context.moveTo(
          Math.round(this.centerx - this.radius),
          this.centery - 5
        );
        this.context.lineTo(
          Math.round(this.centerx - this.radius),
        ...
        for (
          var i = this.centerx - this.radius;
        ...
        for (
          var i = this.centery - this.radius;
          i < this.centery + this.radius;
          i += this.radius / 5
        ) {
      ...
      if (properties.axesTickmarks) {
        ...
        this.context.moveTo(
          this.centerx - 5,
          Math.round(this.centery - this.radius)
        );
        this.context.lineTo(
          this.centerx + 5,
          Math.round(this.centery - this.radius)
  ...
  this.drawRose = function() {
    ...
    if (RGraph.isNull(properties.scaleMax)) {
      ...
      for (var i = 0; i < data.length; ++i) {
        if (typeof data[i] == "number") {
          ...
          properties.variant.indexOf("non-equi-angular") !== -1
        ) {
    ...
    if (
      ...
      for (var i = 0; i < this.data.length; ++i) {
        ...
        var radius =
          ((this.data[i][0] - properties.scaleMin) /
            (this.max - properties.scaleMin)) *
          this.radius;
        ...
        var startAngle =
          this.startRadians * properties.animationRoundrobinFactor -
          RGraph.HALFPI +
          margin;
        var endAngle =
          (this.startRadians + segmentRadians) *
            properties.animationRoundrobinFactor -
          RGraph.HALFPI -
          margin;
        ...
        this.context.arc(
          ...
          properties.animationRoundrobinRadius
      ...
      for (var i = 0; i < this.data.length; ++i) {
        ...
        if (typeof this.data[i] == "number") {
          ...
          var radius =
            ((this.data[i] - properties.scaleMin) /
              (this.max - properties.scaleMin)) *
            this.radius;
          radius = radius * properties.animationGrowMultiplier;
          ...
          var endAngle =
            this.startRadians * properties.animationRoundrobinFactor +
            segmentRadians * properties.animationRoundrobinFactor -
            RGraph.HALFPI -
            margin;
          ...
          for (var j = 0; j < this.data[i].length; ++j) {
            ...
            if (properties.colorsSequential) {
              this.context.fillStyle =
                properties.colors[sequentialColorIndex++];
            }
            ...
            if (j == 0) {
              ...
              var startRadius = 0;
              var endRadius =
                ((this.data[i][j] - properties.scaleMin) /
                  (this.max - properties.scaleMin)) *
                this.radius;
  ...
  this.drawLabels = function() {
    ...
    var radius = this.radius,
      ...
      centery =
        this.centery +
        (properties.variant.indexOf("3d") !== -1
          ? properties.variantThreedDepth
          : 0);
    ...
    if (RGraph.trim(axes).length > 0) {
      RGraph.text({
        ...
        "y": centery,
        "text":
          typeof properties.scaleMin === "number"
            ? RGraph.numberFormat({
                object: this,
                number: Number(properties.scaleMin).toFixed(
                  properties.scaleMin === 0 ? "0" : properties.scaleDecimals
                ),
                unitspre: units_pre,
  ...
  this.drawCircularLabels = function(context, labels, font, size, radius) {
    ...
    for (var i = 0; i < this.data.length; ++i) {
      if (
        ...
        variant.indexOf("non-equi-angular") !== -1
      ) {
        var a = Number(angles[i][0]) + (angles[i][1] - angles[i][0]) / 2;
      } else {
        var a =
          (RGraph.TWOPI / this.data.length) * (i + 1) -
          RGraph.TWOPI / (this.data.length * 2);
        a =
          a -
          RGraph.HALFPI +
          (properties.labelsPosition == "edge"
            ? RGraph.TWOPI / this.data.length / 2
            : 0);
    ...
    for (var i = 0; i < angles.length; ++i) {
      ...
      if (this.context.isPointInPath(mouseXY[0], mouseXY[1])) {
        ...
        if (properties.variant === "non-equi-angular") {
          ...
          angles[i].index = 0;
          angles[i].label =
            RGraph.isArray(properties.labels) &&
            typeof properties.labels[angles[i].dataset] === "string"
              ? properties.labels[angles[i].dataset]
              : null;
        }
  ...
  this.getExploded = function(index, startAngle, endAngle, exploded) {
    ...
    if (typeof exploded == "object" && typeof exploded[index] == "number") {
      explodedx =
        Math.cos((endAngle - startAngle) / 2 + startAngle) * exploded[index];
      explodedy =
        Math.sin((endAngle - startAngle) / 2 + startAngle) * exploded[index];
        ...
        for (var i = 0; i < this.angles.length; ++i) {
          if (i !== shape.sequentialIndex) {
            ...
            if (this.angles[i][2] > 0) {
              ...
              this.path("l % %", this.angles[i][4], this.angles[i][5]);
            }
  ...
  this.parseColors = function() {
    ...
    if (this.original_colors.length === 0) {
      ...
      this.original_colors.highlightStroke = RGraph.arrayClone(
        properties.highlightStroke
      );
      this.original_colors.highlightFill = RGraph.arrayClone(
        properties.highlightFill
      );
    }

    for (var i = 0; i < properties.colors.length; ++i) {
      properties.colors[i] = this.parseSingleColorForGradient(
        properties.colors[i]
      );
    }
    ...
    if (!RGraph.isNull(properties.keyColors)) {
      for (var i = 0; i < properties.keyColors.length; ++i) {
        properties.keyColors[i] = this.parseSingleColorForGradient(
          properties.keyColors[i]
        );
      }
    }
    ...
    properties.segmentHighlightStroke = this.parseSingleColorForGradient(
      properties.segmentHighlightStroke
    );
    properties.segmentHighlightFill = this.parseSingleColorForGradient(
      properties.segmentHighlightFill
    );
  };
  ...
  this.tooltipSubstitutions = function(opt) {
    ...
    var values =
      typeof this.data[indexes[0]] === "number"
        ? [this.data[indexes[0]]]
        : this.data[indexes[0]];

    if (
      properties.variant === "non-equi-angular" &&
      RGraph.isArray(this.data[indexes[0]])
    ) {
  ...
  this.tooltipsFormattedCustom = function(specific, index, colors) {
    ...
    if (typeof this.data[specific.dataset] === "number") {
      ...
      typeof this.data[specific.dataset] === "object" &&
      properties.variant === "non-equi-angular"
    ) {
